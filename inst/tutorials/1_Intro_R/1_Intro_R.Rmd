---
title: "Introduction to R for University of Cologne Political Science"
author: "Ayjeren R. Bekmuratovna, Ingo Rohlfing, Jens Wäckerle"
output: learnr::tutorial
runtime: shiny_prerendered
description: University of Cologne Political Science Introduction to R.
---
...
Both are used to assign a name to a value or a set of values. However, 
in practice you should **almost always** use `<-`.  
...
```{r start-WWI, exercise=TRUE, exercise.lines = 4}
start_of_WWI <- 1914
...
```{r end-WWI, exercise=TRUE, exercise.setup = "start-WWI", exercise.lines = 3}
end_of_WWI <- 1918
```
...
```{r assign_diff, exercise = TRUE, exercise.setup = "end-WWI", exercise.lines = 7}
start_of_WWI <- 1914
end_of_WWI <- 1918
...
```{r object_w_blanks, exercise = TRUE, exercise.setup = "end-WWI", exercise.lines = 8}
start_of_WWI <- 1914
end_of_WWI <- 1918
...
**Naming Vectors**
We can also name vectors in R. When naming a vector, we assign a name to each 
...
Looks like it's seven. We therefore need to create a vector that has eight values.
...
In the example below, both installations are commented out (the line starts with a `#`),
which means it will not be evaluated. This is because the packages are already installed
after you installed this tutorial.
...
## Topic 8: Advanced tidy: the pipe operator
